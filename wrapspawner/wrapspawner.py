...
class ServiceProfilesSpawner(ProfilesSpawner):
    ...
    token_ttl = Int(
        300,
        config = True,
        help = """Time-to-live for the user token, i.e. the time a user token, as generated by this Spawner, shall be
            valid. The Spawner will generate another when necessary."""
    )
    ...
    async def _get_profiles(self):
        ...
        try:
            resp = await http_client.fetch(req)
            profiles_data = json.loads(resp.body.decode("utf-8"))["profiles"]
            if len(profiles_data) == 0:
            ...
            if type(e) == HTTPClientError:
                ...
                self.log.error(f"Could not parse fetch result as JSON. Failed to read {resp.body.decode("utf-8")}.")
            else:
        ...
        self._profiles_fetch = FetchState.COMPLETE
